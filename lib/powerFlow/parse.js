...
/* function buildInverterObservations ({ mapping, date, data }) {
  return Object.entries(data.Inverters).map(([id, inverter]) => {
    return buildInverterObservation({ mapping, date, id, inverter })
  })
} */
...
function buildInverterObservation ({ mapping, date, data }) {
  ...
  output.powerIncoming = Math.max(0, data.Site.P_Akku || 0) + Math.max(0, data.Site.P_PV || 0)
  output.powerOutgoing = get(data, 'Inverters.1.P')
...
function buildSolarObservation ({ mapping, date, data }) {
  ...
  output.powerIncoming = data.Site.P_PV || 0
