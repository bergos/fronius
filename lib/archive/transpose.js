...
function parseTimestamp ({ Start, offset }) {
  ...
  const startLocalized = moment.tz(Start, zone)
  const offsetShifted = startLocalized.clone().add(parseInt(offset), 'seconds')
  const zoneShifted = offsetShifted.add(startLocalized.utcOffset(), 'minutes')
  const tzCorrected = moment.tz(zoneShifted.toISOString().split('Z')[0], zone)
